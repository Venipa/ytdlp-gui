...
const FilterButton: React.FC<FilterButtonProps> = ({ category, count, isSelected }) => (
  ...
  >
    <Button
      ...
      className="relative px-4 py-2 h-auto rounded-full transition-none"
    >
      <span className="flex items-center gap-2 pr-6">
        {isSelected ? `${category}: ${count}` : `Select ${category}`}
      </span>
    ...
    <Popover>
      <PopoverTrigger asChild>
        <FilterButton
          ...
          isSelected={selectedCategoryFilters.length > 0}
        />
      </PopoverTrigger>
  ...
  return (
    <AnimatePresence>
      {selectedFilters.length > 0 && (
        ...
        >
          {filters.filter(filter => selectedFilters.includes(filter.id)).map((filter) => (
