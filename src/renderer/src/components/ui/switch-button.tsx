...
const checkboxVariants = cva("group flex items-start gap-3 rounded-lg p-4 transition-colors cursor-pointer select-none", {
	variants: {
		variant: {
			...
			outline: "border border-input hover:border-muted-foreground/40 hover:bg-muted/60 focus-within:border-muted-foreground/40 focus-within:bg-muted/60",
...
const Checkbox = React.forwardRef<HTMLDivElement, CheckboxProps>(({ checked, defaultChecked, onCheckedChange, variant, className, children, ...props }, ref) => {
	...
	return (
		<div ref={ref} onClick={handleClick} className={cn(checkboxVariants({ variant, className }), props.disabled && "pointer-events-none opacity-60")} {...props}>
