				...
				<div className='relative flex items-center'>
					<div className='min-w-[300px] grow font-mono'>
						{highlightedCode ? (
							...
							<pre className='rounded-md border border-border bg-white p-2 px-4 font-mono dark:bg-black'>{command}</pre>
						)}
					</div>
					<Button variant='outline' size='icon' className='relative ml-2 rounded-md' onClick={copyToClipboard} aria-label={copied ? "Copied" : "Copy to clipboard"}>
						...
						<CopyIcon className={`h-4 w-4 transition-all duration-300 ${copied ? "scale-0" : "scale-100"}`} />
						<CheckIcon className={`absolute inset-0 m-auto h-4 w-4 transition-all duration-300 ${copied ? "scale-100" : "scale-0"}`} />
					</Button>
				</div>
			</div>
		</div>
