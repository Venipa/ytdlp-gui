...
export interface ControlButtonProps extends React.ButtonHTMLAttributes<HTMLButtonElement>, VariantProps<typeof controlButtonVariants> {
	icon: "minimize" | "maximize" | "maximize-out" | "close";
}

const ControlButton = React.forwardRef<HTMLButtonElement, ControlButtonProps>(({ className, variant, size, icon, ...props }, ref) => {
	const Icon = icon === "minimize" ? Minus : icon === "maximize" ? Square : icon === "maximize-out" ? SquareArrowOutDownLeftIcon : X;

	return (
		<button className={cn(controlButtonVariants({ variant, size, className }), "group")} ref={ref} {...props}>
			<Icon className={cn("size-3 relative z-10", icon === "minimize" && "self-end mb-1", icon === "close" && "size-3.5")} />
		</button>
	);
});
