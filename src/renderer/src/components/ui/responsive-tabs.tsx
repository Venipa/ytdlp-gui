...
function TabNavbar({ children, defaultTab, orientation = "horizontal", indicatorPosition = "bottom", className, ...props }: TabNavbarProps) {
	...
	const activeTabLabel = (childrenArray.find((child) => React.isValidElement(child) && child.props.value === activeTab) as React.ReactElement)?.props.children || activeTab;
