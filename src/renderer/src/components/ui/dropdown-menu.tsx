...
>(({ className, inset, preventCloseOnClick, onClick, ...props }, ref) => (
	<DropdownMenuPrimitive.Item
		ref={ref}
		className={cn(
			"relative flex cursor-pointer select-none items-center rounded-sm px-2 py-1.5 text-sm outline-none transition-colors focus:bg-secondary focus:text-secondary-foreground data-[disabled]:pointer-events-none data-[disabled]:opacity-50 [&.active]:bg-primary [&.active]:text-primary-foreground",
			"gap-2 [&_svg]:pointer-events-none [&_svg]:size-4 [&_svg]:shrink-0",
			inset && "pl-8",
			className,
		)}
		{...props}
		onClick={(ev) => {
			if (preventCloseOnClick) ev.preventDefault();
			onClick?.(ev);
		}}
	/>
));
const DropdownMenuItemButton = React.forwardRef<HTMLButtonElement, React.HTMLAttributes<HTMLButtonElement> & {}>(({ className, ...props }, ref) => {
	return (
		<button
			ref={ref}
			className={cn(
...
DropdownMenuItemButton.displayName = DropdownMenuItem.displayName = DropdownMenuPrimitive.Item.displayName;
