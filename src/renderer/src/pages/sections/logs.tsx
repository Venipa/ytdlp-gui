...
const LogItemView = memo(({ date, message, args, type, ...props }: LogEntry & { style?: CSSProperties; key?: string }) => {
	...
	return (
		<div className='grid grid-cols-[100px_48px_1fr] gap-4 items-center text-sm' {...props}>
			...
			<div className={logTypeVariant({ variant: messageType as any, className: "justify-self-center text-xs uppercase font-semibold tracking-wide" })}>{messageType}</div>
				...
				<AutoSizer>
					{({ height, width }) => (
						<List
							...
							overscanRowCount={3}
							noContentRenderer={() => <div className='flex flex-col items-center justify-center text-muted-foreground'>No logs yet.</div>}></List>
					)}
				</AutoSizer>
			</div>
		</div>
