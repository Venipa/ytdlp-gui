  ...
  return (
    <div className="grid items-center grid-cols-[200px_1fr_200px] h-10 flex-shrink-0 overflow-hidden border-t border-t-border flex-auto text-xs text-muted-foreground px-4 select-none">
      ...
      <div className="flex items-center gap-2 justify-end">
        {updateChecking ? <div className='animate-pulse'>Checking for updates...</div> : updateDone ? <ClickableText onClick={() => quitAndUpdate()}>Restart required for update.</ClickableText> : updateProgress && !updateDone ? <div>Downloading... {String(updateProgress.percent).padStart(3, " ")}%</div> : updateAvailable ? <div>Update Available, preparing...</div> : null}
      </div>
    </div>
  )
}
