...
export default function FileSheet({ item: { title, source, url, metaId, filepath, filesize, meta, type, created }, children }: PropsWithChildren<{ item: SelectDownload }>) {
	const { isYoutube, frameSource } = useMemo(() => {
		const isYoutube = isYTDomain.test(source);
		const frameSource = (isYoutube && metaId && `https://www.youtube.com/embed/${metaId}?rel=0&showinfo=0&controls=1&iv_load_policy=3&playsinline=1&fs=0`) || null;
		return { isYoutube, frameSource };
	}, [source]);
	const { mutateAsync: openFile } = trpc.internals.openFile.useMutation();
	const createdAt = useMemo(() => (created && formatDistanceToNow(new Date(created), { includeSeconds: false })) || null, [created]);
