...
export async function queuePromiseStack<T = any>(
  promises: Array<TaskPromise<T>>,
  stackSize: number = 100
) {
  const queueStack = chunk(promises, stackSize).map(
    (promiseChunk) => async () => await Promise.all(promiseChunk.map(async (p) => await p()))
  )
...
export async function queuePromiseAutoStack<T = any>(
  promises: Parameters<typeof queuePromiseStack<T>>[0],
  maxPerStack = 90
) {
  const stackRes = promises.length / maxPerStack
  const stackSize =
    promises.length < maxPerStack ? 10 : clamp(Math.ceil(stackRes * maxPerStack), 1, maxPerStack) // max per stack fallback
...
export async function neverThrow<T>(promise: Promise<T>) {
  return await promise.then(ok).catch(err)
}
export async function resulter<T>(promise: Promise<T>) {
