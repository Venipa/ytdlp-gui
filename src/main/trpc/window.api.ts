...
const getWindowState = async (window: BrowserWindow) => {
	...
	return {
		...pick(window, ["id", "closable", "maximizable", "minimizable", "isFocused", "isMaximized", "isMinimized", "isVisible", "isDestroyed"]),
...
export const windowRouter = router({
	...
	stateChange: publicProcedure.subscription(({ ctx }) => {
		const eventNameMap = ["maximize", "minimize", "closed", "focus", "hide", "show", "blur", "resize", "unmaximize", "ready-to-show", "show", "close"];
	...
	setSize: publicProcedure
		...
		.mutation(({ ctx: { window }, input }) => {
			const { height: currentHeight, width: currentWidth, x: currentX, y: currentY } = window.getContentBounds();
			const widthChanged = input.width !== undefined;
			const heightChanged = input.height !== undefined;
			const { width, height } = Object.assign({}, { width: currentWidth, height: currentHeight }, input);
			const { x, y } = {
