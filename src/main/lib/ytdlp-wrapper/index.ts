...
export default class YTDLWrapper {
    ...
    static async downloadFile(
        ...
        while (currentUrl) {
            const message: IncomingMessage = await YTDLWrapper.createGetMessage(
                currentUrl
            );
    ...
    static async downloadFromGithub(
        ...
        if (!version)
            version = (await YTDLWrapper.getGithubReleases(1, 1))[0].tag_name;
    ...
    exec(
        ...
        ytDlpProcess.stdout.on('data', (data) =>
            YTDLWrapper.emitYoutubeDlEvents(data.toString(), execEventEmitter)
        );
        ...
        ytDlpProcess.on('close', (code) => {
            if (code === 0 || ytDlpProcess.killed)
                ...
                execEventEmitter.emit(
                    'error',
                    YTDLWrapper.createError(code, processError, stderrData)
                );
        });
        ...
        const ytDlpPromise: YTDlpPromise<string> = new Promise(
            (resolve, reject) => {
                ...
                ytDlpProcess = execFile(
                    ...
                    (error, stdout, stderr) => {
                        if (error)
                            reject(YTDLWrapper.createError(error, null, stderr));
                        resolve(stdout);
                    }
                );
                YTDLWrapper.bindAbortSignal(abortSignal, ytDlpProcess);
    ...
    execStream(
        ...
        ytDlpProcess.on('close', (code) => {
            if (code === 0 || ytDlpProcess.killed) {
                ...
                const error = YTDLWrapper.createError(
                    code,
                    processError,
                    stderrData
                );
